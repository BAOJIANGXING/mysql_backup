...
class DatabaseBackup(object):
    ...
    def perform_backup(self, db_name):
        try:
            if db_name != 'ERROR':
                ...
                command = "%s -h%s -P%s -u%s -p%s -S %s --single-transaction --skip-lock-tables --set-gtid-purged=OFF --ssl-mode=DISABLE %s > %s" % \
                ...
                self.logger(f"检测到实例{self.mysql_host}:{self.mysql_port}连接异常，终止备份，请检查配置信息是否正确。")
                self.start_time = time.time()
                self.bksize = '0B'
                self.bkstate = 0
    ...
    def backup_info_reporting(self, dbname):
        ...
        try:
            ...
            self.logger(f"建立数据库{dbname}信息上报连接。")
